...
Features:
- Relative cloud-compatible pathing (Google Cloud Run / Linux ready)
...
# Create tables immediately on startup (Ensures DB works under Gunicorn)
                        ...
                        if id_confidence < 0.50 or clean_predicted != clean_target:
                            ...
                            if disease_model:
                                ...
                                result = {
                                    "status": "error",
                                    "reason": (
                                        f"Health model for {crop_name.capitalize()} missing in models/."
                                    ),
                                }
                    else:
                        result = {
...
def price_crop(crop_name):
    ...
    try:
        ...
        scaler_pkl_path = os.path.join(MODELS_DIR, f"price_{crop_name}_scaler.pkl")
...
# ---------------------------------------------------------------
# SERVER ENTRY POINT (FOR LOCAL TESTING)
# ---------------------------------------------------------------
